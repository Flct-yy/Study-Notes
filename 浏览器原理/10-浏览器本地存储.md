...
- **Max\-Age**：过期时间（相对秒数），如Max\-Age=3600（表示1小时后过期），优先级高于Expires；从Chrome M104版本开始，Max\-Age不能超过400天，防止永久性跟踪。针对跨站（SameSite=None）且未设置 Expires 的 Cookie，Chrome 强制 Max-Age 上限为 400 天；普通第一方 Cookie 不受此硬性限制。
...
sessionStorage与localStorage API完全一致，核心区别在于**生命周期**——sessionStorage是“会话级存储”，数据仅在当前标签页/窗口的生命周期内有效，关闭当前标签页，数据立刻清除；刷新页面（F5）不会清空，但新开标签页（即使是同源）会创建新的sessionStorage空间，关闭浏览器后数据彻底丢失。

通俗来说，sessionStorage就像“临时便签纸”，你可以把当前页面的临时数据（如表单草稿、临时筛选条件）写进去，只要标签不关闭，切换其他标签数据依然保留；**一旦关闭标签页或浏览器，便签纸自动销毁**，不会占用长期存储空间。
